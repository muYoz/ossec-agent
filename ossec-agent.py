#-*- coding: utf-8 -*-
#!/usr/bin/python
import paramiko
import threading
import time
#Linux自动安装脚本
tm = time.strftime('%Y-%m-%d %H:%M:%S',time.localtime(time.time()))
def ssh2(ip, username, passwd, timeout =5):
    cmd = ['rm -rf /etc/yum.repos.d/wazuh.repo\n',
           'cat > /etc/yum.repos.d/wazuh.repo <<\EOF\n'
           '[wazuh_repo]\n'
           'gpgcheck=1\n'
           'gpgkey=https://packages.wazuh.com/key/GPG-KEY-WAZUH\n'
           'enabled=1\n'
           'name=Wazuh repository\n'
           'baseurl=https://packages.wazuh.com/3.x/yum/\n'
           'protect=1\n'
           'EOF\n',
           'yum install -y wazuh-agent',
           'sed -i "s/MANAGER_IP/192.168.1.***/g" /var/ossec/etc/ossec.conf',
           'sed -i "s/disabled>yes/disabled>no/g" /var/ossec/etc/ossec.conf',
           '/var/ossec/bin/agent-auth -m 192.168.1.***  -A %s -P **********' % (ip),
           'systemctl restart wazuh-agent',
           'service wazuh-agent restart']  # 你要执行的命令列表
    try:
        ssh = paramiko.SSHClient()
        ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
        ssh.connect(ip, 22, username, passwd, timeout=timeout)
        for m in cmd:
            stdin, stdout, stderr = ssh.exec_command(m)  # 屏幕输出
            out = stdout.readlines()
            print '%s:' % (ip)
            for o in out:
                print o,
        ssh.close()
        print tm + u"%s :%s + %s done" % (ip, username, passwd)
    except Exception, e:
        print e

if __name__== '__main__':
    usernames = ['root', 'oracle']  # 用户名
    passwds = ['123456', '1qaz2wsx']
    print "Begin......"
    #result = []
    # 将IP一行一个添加到txt
    with open('sship.txt', 'r') as f:
        result = f.readlines()
    for ips in result:
        ip = ips.strip()  # 不加这个会导致下面注册agent的时候ip会出现隔行的巨坑，需要去掉\n
        print ip
        for username in usernames:
            for passwd in passwds:
                passwd = str(passwd.replace('{username}', username))
                a = threading.Thread(target=ssh2, args=(ip, username, passwd))
                a.start()
                time.sleep(1)
